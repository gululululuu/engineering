...
dataConversionUtil.investigationToExcel = function (items, tables) {
  let num = 0
  let workBook = XLSX.utils.book_new()
  items.forEach(item => {
    let tableHeader = tables[num++]
    let dataList = tables[num++]
    // excel的表头和数据
    let aoa = []
    // aoa的数据格式：[[],[],[],[]]   数组的第一个子数组可以指定为表头  根据情况而定
    aoa = tableHeader.concat(dataList)
    ...
    var maxLen = dataList[0].length - 1
    workSheet['!merges'] = [
      // 设置单元格合并
      {s: {r: 0, c: 0}, e: {r: 0, c: maxLen}},
      {s: {r: 1, c: 2}, e: {r: 1, c: maxLen}}
    ]
    // 单元格宽度
    let width = []
    // 单元格高度
    ...
    console.log(workSheet)
    // 把数据写到工作簿中
    XLSX.utils.book_append_sheet(workBook, workSheet, item[0])
  })
  // 如果一个工作工作簿中有多个工作表，可以修改参数类型并遍历添加，期中workBook是同一个，workSheet和sheet根据自己的需求添加，
  // 比如在此处添加第二个工作表叫‘第二张表’，把数据封装好后，数据格式同上,假如数据叫workSheet2，执行下面代码，工作簿就会多一张工作表叫‘第二张表’
  // XLSX.utils.book_append_sheet(workBook,workSheet2,'第二张表')
...
dataConversionUtil.scoreToExcel = function (workItems, workTables, investigationItems, invesTables, examHeader, examDataList, examName) {
  ...
  investigationItems.forEach(item => {
    let tableHeader = invesTables[num++]
    let dataList = invesTables[num++]
    // excel的表头和数据
    let aoa = []
    // aoa的数据格式：[[],[],[],[]]   数组的第一个子数组可以指定为表头  根据情况而定
    aoa = tableHeader.concat(dataList)
